...
In the following subchapters the methodology is described in detail. The data sources as well as their import in an R-environment, the data processing and evaluation as well as the method for the creation of a time model are shown.

In this report (HTML) only the most relevant outputs are shown in order to confine to the essential results. When using the provided R-Markdown (.Rmd), the underlying R code can be followed in the corresponding method and result chapters.
...
As shown in figure x below, only the DEM-rasters of the observation area have been imported automatically. The points additionally show all the x- and y- coordinates that have been tracked by the athlete among all activities. They are all covered by the imported DEM.
...
#### 2.2.2 Calculation of Euclidian Steplength
...
The distance between the distance derived by the sportwatch and distance calculation by euclidian distance was compared and presented as deviation in the results.
...
tophal: ich habe das hist noch in einen ggplot umgewandelt, so sehen alle Grafiken einheitlich aus :)
...
#### 3.1.2 Altitude by Pressure vs. Altitude from DEM <a name="subparagraph1"></a>
