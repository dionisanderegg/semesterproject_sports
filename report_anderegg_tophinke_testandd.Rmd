...
2.  [Methods](#methods)
    ...
    2.2 [Preparation and evaluation of data ](#subparagraph3)
    
      2.2.1  [Tracked distance vs. calculated distance](#subparagraph3.1)
      2.2.2  [Compare recorded distance with Euclidean Steplength](#subparagraph3.2)
      2.2.3  [Rolling windows](#subparagraph3.2)
      2.2.4  [Define static and moving points](#subparagraph3.3)
      2.2.4  [Define outliers](#subparagraph3.4)

    2.3 [Model individual time requirement](#subparagraph4)

3.  [Results](#results)

    3.1 [Comparison data from sportwacht and calculated values](#subparagraph5)
    
    3.1.1  [Altitude by pressure vs. altitude from DEM](#subparagraph5.1)
    3.1.2  [Tracked distance vs. calculated distance](#subparagraph5.2)
     
    3.2 [Research Question 1](#subparagraph6)
    
    3.2.1  [Visualization of the tracks on map](#subparagraph6.1)
    3.2.2  [Variables affecting the time requirement](#subparagraph6.2)
    3.2.3  [Heart rate patterns](#subparagraph6.3)

    3.3 [Research Question 2](#subparagraph7)
    
    3.3.1  [Heart rate patterns](#subparagraph7.1)
    3.3.2  [Heart rate patterns](#subparagraph7.2)

    3.4 [Research Question 3](#subparagraph8)
    
    3.4.1  [Simple model](#subparagraph8.1)
    3.4.2  [Exhaustion model](#subparagraph8.2)
    3.4.3  [Comparison of the models and the measurement](#subparagraph8.3)
    3.4.4  [Validation of the Exhaustion Model](#subparagraph8.4)
    3.4.5  [Visualization of Exhaustion Model](#subparagraph8.5)

4.  [Disussion](#discussion)

    4.1 [Accuracy of the Data](#subparagraph9)
    4.2 [Limits and Accuracy of the Model](#subparagraph10)
    4.3 [Further studies](#subparagraph11)
...
### 2.2.1 Compare elevation by pressure with DEM <a name="subparagraph3.1"></a>
...
As shown in Figure 1 below, only the DEM-rasters of the runned area have been imported automatically. The points additionally show all the x- and y- coordinates that have been tracked by the athlete among all activities. They are all covered by the imported DEM.
...
### 2.2.3 Rolling windows <a name="subparagraph3.3"></a>

In this chapter a rolling window with different k-values was generated in order to smooth the time step-values. In figure 2 violin plots from all tracks from athlete 1 are shown. Further in figure 3 all smoothed speed curves are collected and compared to each other. Enhanced_speed_00 is the curve without rolling window (enhanced_speed multiplied by 3.6 to get the unit km/h). Since low values (around 0 km/h) are important to pick out the breaks in further steps, these values must not be lost. For this reason k=3 was defined and used for further calculations.
...
### 2.2.4 Define static and moving points <a name="subparagraph3.4"></a>

This chapter is about distinguishing the pauses from the running parts. Since it is also possible to walk around during a break, a threshold in speed has to be defined. Therefore a histogram with all speed values from Athlete 1 was generated. From visual inspection oof the histogram and together with the data from the violin plots from chapter x, the threshold was defined at \<= 3.25 km/h. Therefore all speed values \<=3.25km/h were labeld as static = TRUE.
...
### 2.2.5 Define outliers <a name="subparagraph3.5"></a>
...
# 3. Results <a name="results"></a> {#results}

## 3.1. Comparison data from sportwacht and calculated values <a name="subparagraph5"></a>

### 3.1.1 Altitude by pressure vs. altitude from DEM <a name="subparagraph5.1"></a>
...
In the case of absolute altitude, there are only a few major differences between the two data sources. For example, activity 9 shows a comparatively large deviation at the beginning, see Figure 4.

```{r echo=FALSE, fig.cap="Figure 4: Comparison of elevation calculation based on DEM and measurement based on air pressure of the sportwatch", fig.height=10, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
### 3.1.2 Tracked distance vs. calculated distance <a name="subparagraph5.2"></a>

The comparison between the recorded distances and those from Euclidean Distance shows that in relation to the whole track the values deviate only between approx. -1% and +1%. Therefore, the distances tracked by the sports watch can be further used.

```{r echo=FALSE, fig.cap="Figure 6: histogram for distance_diff (calculated distance - recorded distance/ full track distance)", message=FALSE, warning=FALSE, paged.print=FALSE}
...
## 3.2 Research Question 1 <a name="subparagraph6"></a>

### 3.2.1 Visualization of the tracks on map <a name="subparagraph6.1"></a>
...
The Package leaflet offers an interactive option for visualizing activities on a map, which is particularly impressive visually. In the following figures 7 and 8, two selected activities are visualized using leaflet. By zooming in, the accuracy of the GPS fixes can be analysed visually.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 7: Visualization of a track on a map using package leaflet"}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 10: Visualization of a track on a map using package ggmap. The color scale represents the elevation at the coordinate."}
...
### 3.2.2 Variables affecting the time requirement <a name="subparagraph6.2"></a>

To answer research question 1, an exploratory data analysis (EDA) of the collected tracks was first conducted.

The Shiny app (see figure 11) was programmed to browse through the different tracks of one athlete. Via dropdown menu it is possible to choose between the different collected tracks:

```{r echo=FALSE, fig.cap="Figure 11:", fig.height=10, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
### 3.2.3 Summary of tracks <a name="subparagraph6.3"></a>
...
## 3.3 Research Question 2 <a name="subparagraph7"></a>
...
### 3.3.1 Moving/breaking <a name="subparagraph7.1"></a>
...
The following boxplot (figure 12) shows that mainly tracks of 1 h were collected. For the model in Research Question 3, this means that the prediction works best for routes up to 1h. Longer routes (up to 3h) become inaccurate, because only few data are available. Track 1 and 19 are not excluded here as outliers, but it is pointed out that prediction for longer tracks (between 2-3h) can become inaccurate. 
 
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 12: Identification of outliers"}
full_short %>%
...
In figure 13 we see another shiny app which allows to switch between the different tracks in order to vizualize running and breaking sections. 

```{r echo=FALSE, fig.cap="Figure 13:", fig.height=10, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
Figure 14 shows the relationship between running time and break-time in minutes. The longer the running time, the longer the breaks required. Track 8 was excluded as it involves an extraordinary long pause (outlier).
...
```{r echo=FALSE, fig.cap="Figure 14: relationsip between running and pause time", message=FALSE, warning=FALSE, paged.print=FALSE}
...
In following figure 15 the parts which include highrates >85% are highlighted. 

If red areas appear (see Figure 15), a reduction in speed is recommended. 

```{r fig.cap="Figure 15: tracks with indicated high pulse sections (colored in red) ", message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
## 3.4 Research Question 3 <a name="subparagraph8"></a>
...
#### 3.4.1 Simple model <a name="subparagraph8.1"></a>
...
The above described simple model without exhaustion predicts the time requirement for all activities with some deviation to the measured time requirement. As figure 16 shows, the time estimation meets the measured time requirement for some activities pretty well, whereas the time requirement especially for long activities (measured time requirement) are mostly underestimated. This shows the effect of the exhaustion.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 16: Time estimation of the simple model without exhaustion in comparison with measured time requirement for all tracks of the athlete ordered by decreasing time requirement of the measurement."}
...
#### 3.4.2 Exhaustion model <a name="subparagraph8.2"></a>
...
#### 3.4.3 Comparison of the models and the measurement <a name="subparagraph8.3"></a>

Figure 19 additionally shows the measured time requirement per track in descending order as well as the prediction of both models. The exhaustion model seems to predict the time requirement more precise for the most of the activities.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 19: Measured time requirement and both model estimations per track, ordered by descending distances."}
zf %>% 
...
#### 3.4.4 Validation of the Exhaustion Model <a name="subparagraph8.4"></a>
...
### 3.4.5 Visualization of Exhaustion Model <a name="subparagraph8.5"></a>

Assuming a range of validity of the model with distances between 3 and 32 kilometers and positive altitude differences between 0 and 1000 meters, the time required for activities can be classified on the basis of the model and displayed on a graph. Figure 20 shows such a graph, where the predicted time demand can be read out on the vertical z-axis under different conditions (distances and altitude differences). However, the extremely broad scope of this observation means that the time prediction can only serve as a rough basis for planning. Effective times will deviate, among other things, due to the non-linearity of fatigue and environmental parameters. These aspects will be examined in more detail in the discussion.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Figure 20: 3D-Model of the time estimation by the exhaustion model depending of various distances and hight differences. Hight differences are always calculated for both, upwards and downwards."}
...
##4.1 Accuracy of the Data <a name="subparagraph9"\></a>
...
Finally, however, all relevant parameters depend on the accuracy of the GPS fixes. Figure 20 shows that the deviations between the GPS fixes and the paths can sometimes be large. A visual analysis of tracks on the map revealed large differences, especially in areas with forest cover. These differences influence all predictors of the model (distance directly and altitude differences indirectly via the DEM).

## 4.2 Limits and Accuracy of the Model <a name="subparagraph10"></a>
...
## 4.3 Further studies<a name="subparagraph11"></a>
...
- emotions
- or similar
can also influence the time needed for a certain distance. These factors were not considered in this work. 

## 5 References <a name="references"></a> {#references}
