...
# Overwrite timelag to prevent from huge timelags between different activities:
full <- full %>%
...
speed_mean <- mean(full$enhanced_speed)

full<- full %>% 
  ggplot()+
  geom_histogram(aes(enhanced_speed))+
  geom_vline(data=mu, aes(xintercept=enhanced_speed),
             linetype="dashed")

```

=======
>>>>>>> ff098b823d4f19b97b2715c9dc1a284f3041ac3c
...
#create table with overview of mean, sd, var for different tracks and different moving windows
...
#plot mean speedn and variance per track
...
#calculate further speed via steplength/ timelag
...
full_short <- tracks %>% 
  ...
  filter(static == FALSE) %>% 
    mutate(duration_moving = as.numeric(difftime(max(timestamp), min(timestamp), units = "hours"))) %>% 
  ungroup() %>% 
...
full_short %>%
  ...
  ggplot(aes(x = activity_ID, y = duration_moving)) +
    geom_boxplot() +
    geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.2)+
  theme_bw()+
    labs(title="moving duration of the tracks",
...
#track 1,8 and 14 has to be controlled for possible outliers
...
#date of athlete 1
birth_athelete1 <- as.Date('1992-10-20')
now <- Sys.time()
