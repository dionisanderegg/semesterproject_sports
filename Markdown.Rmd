...
#choose k-value for your data --> statistische Auswertung oder aus Grafik 



###1.4.3 Rolling window (smoothen) (alissa --> müsste plot noch anpassen, sowieso doppelt)


rolling_window_function <- function(smoothing_window){
  
  full_k <- mutate(full, movingwindow = rollmean(enhanced_speed_kmh, k=smoothing_window, fill=NA, align="left"))
...
full_sf <- full_sf %>%
  group_by(activity_ID) %>%
  mutate(speed_calculated = 3.6*(steplength/timelag)) 
...
compare_speed <- full_sf %>%
  filter(activity_ID == random_tracks) %>%
  ggplot() +
  geom_point(aes(timestamp, enhanced_speed_kmh, col = "blue", alpha=0.1)) +
  geom_point(aes(timestamp, speed_calculated, col = "red", alpha=0.1)) +
  theme_bw() +
  theme(legend.position = "bottom") +
  facet_wrap(~activity_ID, scales = "free") +
  labs(x = "\n time", y = "speed\n")
...
#make a temporal window v of 10 minutes, i.e. a window size of 10 positions (n±5).
tracks <- full_sf
...
max_heart_rate_athlete <-max(tracks$heart_rate)
high_rate <- (max_heart_rate_athlete*0.8)
