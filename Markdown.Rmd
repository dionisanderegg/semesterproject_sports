...
compare_speed <- full_sf %>%
  filter(activity_ID == random_tracks) %>%
  ggplot() +
  geom_point(aes(timestamp, distance, col = "speed_enhanced", alpha=0.1)) +
  geom_point(aes(timestamp, steplenght_sum, col = "speed_calculated", alpha=0.1)) +
  theme_bw() +
  theme(legend.position = "bottom") +
  facet_wrap(~activity_ID, scales = "free") +
  labs(x = "\n time", y = "distance in meters\n")
...
max_heart_rate_athlete*0.8


tracks_heart <- tracks %>% 
  group_by(activity_ID) %>% 
    mutate(high = heart_rate > (0.8*(max_heart_rate_athlete)), na.rm = TRUE)
