...
### 1.4.1 calculate timelag and individual activity_ID for each track
...
###1.4.3 Rolling window (smoothen) (alissa)


rolling_window_function <- function(smoothing_window){
  
  full_k <- mutate(full, movingwindow = rollmean(full$enhanced_speed_kmh, k=smoothing_window, fill=NA, align="left"))
...
# calculate Euclidian step length
full_sf <- full_sf %>%
  group_by(activity_ID) %>%
  mutate(
...
compare_euclidian.distance_measurement <- full_sf %>%
...
ggplotly(compare_euclidian.distance_measurement)
```
...
## 1.3 evaluate altitude with DEM => After creating full_sf!
...
# Calculate altitide difference between pressure measurement and DHM layer.
...
The following shows graphically how the variables speed, altitude difference and heart rate behave.
```{r}
...
plot_speed_altitude_heartrate_function <- function(track_n){
  
  for (s in 1:length(full_sf_join$activity_ID)) {
  plotName <- paste0("p", s)
} ->   full_long %>%
  filter(activity_ID == track_n) %>%
...
ui <-shinyUI(fluidPage(selectInput("selectPlot", "Choose desired plot", choices=paste0("p", 1:length(full_sf_join$activity_ID))), plotlyOutput("plot")))
